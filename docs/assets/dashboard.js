...
async function initDashboard(path){const data=await loadJSON(path),series=data.series||[];renderChart(data.dates||[],series);renderSummary(series);renderTable(series);renderStrategyDetails(data.strategies||{});renderDecisionHistory(data.decision_history);setStatus(data);}
