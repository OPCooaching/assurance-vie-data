...
"ChatGPT D — Sélection mensuelle et protection hebdomadaire":"#e05263",
...
function readableStrategyStatus(status){return ({research_hypothesis:"Hypothèse à tester",backtest:"Backtest en cours",live_paper_tracking:"Suivi simulé",paused:"Suspendue"}[status]||status||"Statut non précisé");}
...
async function initBacktestResults(path){
  ...
  const rows=(data.results||[]).map(r=>{
    const m=r.metrics||{}, provisional=r.data_quality==="provisional_revised_macro_data";
    const total=Number.isFinite(Number(m.final_value))?Number(m.final_value)/100-1:NaN;
    return '<tr><td><strong>'+htmlEscape(r.label||r.strategy_id||"—")+'</strong><small class="backtest-version">'+htmlEscape(r.strategy_id||"")+'</small></td><td>'+htmlEscape(m.start||"—")+' → '+htmlEscape(m.end||"—")+'</td><td>'+backtestPct(total)+'</td><td>'+backtestPct(m.annualized_return)+'</td><td>'+backtestPct(m.annualized_volatility)+'</td><td>'+backtestPct(m.max_drawdown)+'</td><td>'+backtestNumber(m.average_holdings)+'</td><td>'+backtestNumber(m.average_effective_holdings)+'</td><td>'+htmlEscape(m.decisions||"—")+'</td><td>'+(provisional?'Provisoire : données macro révisées':'Prix disponibles à chaque décision')+'</td></tr>';
  }).join("")||'<tr><td colspan="10">Aucun backtest disponible.</td></tr>';
  box.innerHTML='<div class="table-wrap"><table><thead><tr><th>Stratégie</th><th>Période</th><th>Gain total</th><th>Rendement annuel</th><th>Volatilité</th><th>Pire baisse</th><th>Lignes moy.</th><th>Diversification effective</th><th>Revues</th><th>Limite</th></tr></thead><tbody>'+rows+'</tbody></table></div><p class="muted backtest-note">« Lignes moy. » est le nombre moyen de supports retenus. La diversification effective tient compte des poids : elle baisse si quelques lignes portent davantage de risque.</p>';
}
