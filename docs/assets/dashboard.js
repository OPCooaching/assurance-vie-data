...
function backtestPct(value){return Number.isFinite(value)?(Number(value)*100).toFixed(1)+" %":"—";}
async function initBacktestResults(path){const data=await loadJSON(path),box=document.querySelector("#backtest-results");if(!box)return;box.innerHTML='<div class="table-wrap"><table><thead><tr><th>Stratégie</th><th>Période</th><th>Valeur finale</th><th>Rendement annuel</th><th>Volatilité</th><th>Pire baisse</th><th>Décisions</th><th>Limite</th></tr></thead><tbody>'+((data.results||[]).map(r=>{const m=r.metrics||{},provisional=r.data_quality==="provisional_revised_macro_data";return '<tr><td><strong>'+htmlEscape(r.strategy_id)+'</strong></td><td>'+htmlEscape(m.start||"—")+' → '+htmlEscape(m.end||"—")+'</td><td>'+htmlEscape(Number(m.final_value).toFixed(1))+'</td><td>'+backtestPct(m.annualized_return)+'</td><td>'+backtestPct(m.annualized_volatility)+'</td><td>'+backtestPct(m.max_drawdown)+'</td><td>'+htmlEscape(m.decisions||"—")+'</td><td>'+(provisional?'Provisoire : données macro révisées':'Prix disponibles à chaque décision')+'</td></tr>';}).join('')||'<tr><td colspan="8">Aucun backtest disponible.</td></tr>')+'</tbody></table></div>';}
