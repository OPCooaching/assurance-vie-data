...
"Bernard origine":"#f4f7fb",
"ChatGPT A — Momentum mensuel multi-horizons":"#ffd166",
"ChatGPT B — Exposition pilotée par le risque":"#35c78a",
"ChatGPT C — Régime macro-financier":"#a78bfa",
"ChatGPT D — Sélection mensuelle et protection hebdomadaire":"#e05263","ChatGPT G — Stratégie qui réapprend chaque semaine":"#4cc9f0",
"60/40 mondial adapté":"#457b9d","Harry Browne adapté":"#2a9d8f","Faber Trend 10 mois":"#8f5fbf","Momentum académique 12 mois":"#f4a261",
"Claude A — Socle mondial et satellites":"#38bdf8","Claude B — Risque cible constant":"#6c63ff","Claude C — Tendance confirmée par l’ampleur":"#2a9d8f","Claude D — Momentum multi-horizon":"#ff9f1c","Claude E — Momentum sous garde-fou":"#ec4899",
"Repère académique (moyenne)":"#2a9d8f","ChatGPT (moyenne des 4)":"#a78bfa","Claude (moyenne des 5)":"#f4a261"};
const FALLBACK=["#38bdf8","#ffd166","#35c78a","#a78bfa","#e05263"];
...
const euro=x=>Number.isFinite(x)?new Intl.NumberFormat("fr-FR",{style:"currency",currency:"EUR",maximumFractionDigits:0}).format(x):"—";
...
function renderSummary(series,startEur){const box=document.querySelector("#summary");if(box)box.innerHTML=series.map((s,i)=>{const m=metrics(s.values),value=startEur&&Number.isFinite(m.last)?euro(startEur*m.last/100):(Number.isFinite(m.last)?"base "+m.last.toFixed(2):"—");return '<article class="metric"><span class="eyebrow">'+swatch(s.label,i)+s.label+'</span><b>'+pct(m.since)+'</b><small>Depuis le 09/09/2026 · '+value+'</small></article>';}).join("");}
function renderTable(series,startEur){const body=document.querySelector("#metrics-body");if(body)body.innerHTML=series.map((s,i)=>{const m=metrics(s.values),value=startEur&&Number.isFinite(m.last)?euro(startEur*m.last/100):(Number.isFinite(m.last)?m.last.toFixed(2):"—");return '<tr><td>'+swatch(s.label,i)+s.label+'</td><td>'+pct(m.since)+'</td><td>'+pct(m.r5)+'</td><td>'+pct(m.r20)+'</td><td>'+pct(m.dd)+'</td><td>'+pct(m.vol20)+'</td><td>'+value+'</td></tr>';}).join("");}
...
function renderChart(labels,series,startEur){const canvas=document.querySelector("#comparison-chart");if(!canvas)return;renderLegend(series);if(activeChart)activeChart.destroy();activeChart=new Chart(canvas,{type:"line",data:{labels,datasets:series.map((s,i)=>({label:s.label,data:startEur?s.values.map(value=>Number.isFinite(value)?startEur*value/100:null):s.values,borderColor:colorFor(s.label,i),backgroundColor:colorFor(s.label,i),borderWidth:s.label==="Bernard origine"?5:2.5,pointRadius:labels.length<12?3:0,pointHoverRadius:6,tension:.28,spanGaps:true,fill:false}))},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:"index",intersect:false},plugins:{legend:{display:false},tooltip:{padding:12,callbacks:{label:c=>c.dataset.label+": "+(startEur?euro(c.parsed.y):c.parsed.y.toFixed(2))}}},scales:{x:{grid:{display:false},ticks:{color:"#99abb8",maxRotation:0}},y:{grid:{color:"rgba(148,163,184,.14)"},ticks:{color:"#99abb8"},title:{display:true,text:startEur?"Valeur estimée (€)":"Indice base 100",color:"#99abb8"}}}}});}
function readableStrategyStatus(status){return ({research_hypothesis:"Hypothèse à tester",backtest:"Test historique",live_paper_tracking:"Suivi simulé",paused:"Suspendue"}[status]||status||"Statut non précisé");}
function renderStrategyDetails(strategies){const box=document.querySelector("#strategy-details");if(!box)return;const list=Array.isArray(strategies)?strategies:Object.entries(strategies||{}).map(([id,s])=>({...s,id}));box.innerHTML=list.map((s,i)=>{const holdings=(s.holdings||[]).map(h=>'<li><span>'+h.name+'</span><b>'+Number(h.weight_pct).toFixed(2)+' %</b></li>').join("");return '<article class="strategy"><div class="strategy-title">'+swatch(s.label,i)+'<div><h3>'+s.label+'</h3><small>'+[s.version,readableStrategyStatus(s.status)].filter(Boolean).join(" · ")+'</small></div></div>'+(s.resume||s.short?'<p>'+(s.resume||s.short)+'</p>':"")+(s.objectif||s.objective?'<p><strong>But.</strong> '+(s.objectif||s.objective)+'</p>':"")+(s.regle||s.main_rule?'<p><strong>Règle de sélection et de remplacement.</strong> '+(s.regle||s.main_rule)+'</p>':"")+(s.frequence?'<p><strong>Quand cela peut changer.</strong> '+s.frequence+'</p>':"")+(s.faiblesse?'<p class="muted"><strong>Point de vigilance.</strong> '+s.faiblesse+'</p>':"")+(holdings?'<div class="holdings"><h4>Allocation de la dernière revue</h4><ul>'+holdings+'</ul></div>':'<p class="muted">Allocation en attente d\'un suivi simulé.</p>')+(s.statut_public?'<p class="status-note">'+s.statut_public+'</p>':"")+'</article>';}).join("")||'<p class="muted">Aucune stratégie documentée.</p>';}
...
function valuationStatus(data){const e=document.querySelector("#status");if(!e)return;const valuation=data.valuation||{};const last=valuation.latest_provisional_date||(data.dates||[]).at(-1);const checked=data.generated_at?new Date(data.generated_at).toLocaleString("fr-FR"):"—";if(!last){e.textContent="Aucune valorisation publiée pour le moment.";return;}if(valuation.latest_status==="provisional"){const waiting=(valuation.pending_supports||[]).map(item=>htmlEscape(item.support_name)+" (cours officiel du "+formatDate(item.latest_official_close_date)+")").join(" · ");e.innerHTML="Dernière valeur estimée : <strong>"+formatDate(last)+"</strong> — <strong>provisoire</strong>. "+(waiting?"Cours encore attendus : "+waiting+". ":"")+"Le dernier cours officiel connu est conservé temporairement pour ces supports ; ce point sera corrigé automatiquement dès leur publication. Dernière valorisation entièrement consolidée : "+formatDate(valuation.latest_consolidated_date)+". Vérifiée par le dernier traitement : "+checked+".";return;}e.innerHTML="Dernière valorisation entièrement consolidée : <strong>"+formatDate(last)+"</strong>. Vérifiée par le dernier traitement : "+checked+".";}
function allocationText(allocation){return Object.entries(allocation||{}).map(([asset,weight])=>htmlEscape(asset)+" : "+htmlEscape(Number(weight).toFixed(1))+" %").join(" · ")||"—";}
function renderDecisionHistory(decisions){const body=document.querySelector("#history-body");if(!body)return;body.innerHTML=(decisions||[]).map(d=>'<tr><td>'+htmlEscape(formatDate(d.date))+'</td><td>'+htmlEscape(d.strategy_id||"—")+'</td><td>'+htmlEscape(d.strategy_version||"—")+'</td><td>'+htmlEscape(d.decision_type||"—")+'</td><td>'+htmlEscape(d.rationale||"—")+'</td><td>'+allocationText(d.target_allocation_percent)+'</td></tr>').join("")||'<tr><td colspan="6">Aucune décision enregistrée : les stratégies sont encore au stade du test.</td></tr>';}
function chatgptSeriesOrder(left,right){const order=label=>{if(label==="Bernard origine")return 0;const match=String(label).match(/^ChatGPT ([A-Z]) /);return match?match[1].charCodeAt(0)-64:99;};return order(left.label)-order(right.label);}
async function initDashboard(path){const data=await loadJSON(path),series=(data.series||[]).slice().sort(chatgptSeriesOrder),startEur=data.start_eur||null;renderChart(data.dates||[],series,startEur);renderSummary(series,startEur);renderTable(series,startEur);renderStrategyDetails(data.strategies||{});renderDecisionHistory(data.decision_history);valuationStatus(data);}
...
function backtestPct(value){return Number.isFinite(Number(value))?(Number(value)*100).toFixed(1)+" %":"—";}
function backtestNumber(value){return Number.isFinite(Number(value))?Number(value).toFixed(1):"—";}
async function initBacktestResults(path){
  const data=await loadJSON(path),box=document.querySelector("#backtest-results");
  if(!box)return;
  const rows=(data.results||[]).map(r=>{
    const m=r.metrics||{}, total=Number.isFinite(Number(m.final_value))?Number(m.final_value)/100-1:NaN;
    return '<tr><td><strong>'+htmlEscape(r.label||r.strategy_id||"—")+'</strong><small class="backtest-version">'+htmlEscape(r.strategy_id||"")+'</small></td><td>'+htmlEscape(m.start||"—")+' → '+htmlEscape(m.end||"—")+'</td><td>'+backtestPct(total)+'</td><td>'+backtestPct(m.annualized_return)+'</td><td>'+backtestPct(m.annualized_volatility)+'</td><td>'+backtestPct(m.max_drawdown)+'</td><td>'+backtestNumber(m.average_holdings)+'</td><td>'+backtestNumber(m.average_effective_holdings)+'</td><td>'+htmlEscape(m.decisions||"—")+'</td></tr>';
  }).join("")||'<tr><td colspan="9">Aucun backtest disponible.</td></tr>';
  box.innerHTML='<div class="table-wrap"><table><thead><tr><th>Stratégie</th><th>Période</th><th>Gain total</th><th>Rendement annuel</th><th>Volatilité</th><th>Pire baisse</th><th>Lignes moy.</th><th>Diversification effective</th><th>Revues</th></tr></thead><tbody>'+rows+'</tbody></table></div><p class="muted backtest-note">Les tests A et B utilisent les prix. Les tests C, D et G utilisent aussi le contexte macro. Certaines valeurs macro anciennes ont pu être corrigées après leur publication : leurs résultats servent donc à comparer les méthodes, pas à annoncer une certitude sur le passé.</p>';
}
