...
function setStatus(data){const e=document.querySelector("#status");if(!e)return;const last=(data.dates||[]).at(-1);const checked=data.generated_at?new Date(data.generated_at).toLocaleString("fr-FR"):"—";e.innerHTML=last?"Dernière valorisation réellement disponible : <strong>"+formatDate(last)+"</strong>. Vérifiée par le dernier traitement : "+checked+".":"Aucune valorisation réelle publiée pour le moment.";}
